...
actor Chord
  ...
  be report_hops(hops: USize) =>
    ...
    //env.out.print("Progress: " + completed_requests.string() + "/" + 
      //(num_nodes * num_requests).string() + " requests completed (Hops: " + hops.string() + ")")
...
actor Node
  ...
  new create(id': USize, m': USize, chord': Chord tag) =>
    id = id'
    m = m'
    finger = Array[USize].init(0, m')
    successor = id
    chord = chord'
    rand = Rand
  ...
  be find_successor(key: USize, requester: Node tag, i: (USize | None) = None) =>
    ...
    if is_between(key, id, successor) then
      match i
  ...
  be start_requests(num: USize) =>
    if requests_sent < num then
      let key = rand.int(((1 << m) - 1).u64()).usize()
  ...
  fun ref closest_preceding_node(key: USize): USize =>
    ...
    while i > 0 do
      ...
      try
        ...
        if (finger_id != id) and is_between(finger_id, id, key) then
          return finger_id
        end
      end
    end
